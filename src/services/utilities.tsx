...
export const displaySelectTime = (selectedTime: any) => {
...
const calculateTimimg = (h: number) => {
  let months = 0,
    weeks = 0,
    daysCT = 0,
    hours = 0;
  ...
  return {months, weeks, daysCT, hours};
};

const prepareRemainingPeriod = (
  months: string | number,
  weeks: string | number,
  daysPRP: string | number,
) => {
...
const prepareElapsedPeriod = (
  months: string | number,
  weeks: string | number,
  daysPEP: string | number,
  hours: string | number,
) => {
  let elapsed_period = '';
...
export const calculateTime = (date: any, selectedTime: string) => {
  let session_period: string;
  ...
  if (date && selectedTime) {
    ...
    if (diff_hours > 0) {
      let {months, weeks, daysCT} = calculateTimimg(diff_hours);
      session_period = prepareRemainingPeriod(months, weeks, daysCT);
    } else {
      let {months, weeks, daysCT, hours} = calculateTimimg(diff_hours * -1);
      session_period = prepareElapsedPeriod(months, weeks, daysCT, hours);
    }
